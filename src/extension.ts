...
async function getPublicIPFromIpify(): Promise<string> {
...
async function getTunnelPassword(): Promise<string> {
  return new Promise((resolve, reject) => {
    https
      .get("https://loca.lt/mytunnelpassword", (res) => {
        ...
        res.on("end", () => {
          try {
            // HTML 응답에서 IP 주소 추출
            const ipMatch = data.match(
              /your tunnel password is: <b>([^<]+)<\/b>/i
            );
...
export function activate(context: vscode.ExtensionContext) {
  ...
  // 터널 시작 명령
  let startTunnel = vscode.commands.registerCommand(
    "remote-debug-tunnel.startTunnel",
    async () => {
      if (currentTunnel) {
      ...
      try {
        vscode.window.showInformationMessage(
          `포트 ${port}에서 터널을 시작하는 중...`
        );

        const tunnelOptions: localtunnel.TunnelConfig & { port: number } = {
          port: port,
        };
        ...
        currentTunnel = await localtunnel(tunnelOptions);
        ...
        try {
          ...
          const action = await vscode.window.showInformationMessage(
            `터널이 생성되었습니다!\n\nURL: ${url}\n\n⚠️ 주의: 다른 사람이 접속하려면 터널 비밀번호(공개 IP)가 필요합니다.\nhttps://loca.lt/mytunnelpassword 에서 확인하세요.`,
            "URL 복사",
            "브라우저에서 열기",
            "QR 코드 생성"
          );
        ...
        // 터널 에러 핸들링
        currentTunnel.on("error", (err: Error) => {
  ...
  // 터널 중지 명령
  let stopTunnelCommand = vscode.commands.registerCommand(
    "remote-debug-tunnel.stopTunnel",
    () => {
      if (!currentTunnel) {
  ...
  let showStatus = vscode.commands.registerCommand(
    "remote-debug-tunnel.showStatus",
    () => {
      if (currentTunnel) {
        const url = currentTunnel.url;
